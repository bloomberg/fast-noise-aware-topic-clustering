...
def write_cluster_samples(clustering_model, labels, write_path, max_write_per_cluster=10):
    '''
...
def write_labels_and_assignments(labels_and_assignments_file, clustering_model, data_labels):
    '''
...
def _get_output_basename(output_dir, cluster_algorithm, clustering_run_id):
    return os.path.join(
        output_dir, 
        f"{cluster_algorithm}_{clustering_run_id}"
    )


def save_results(args, data_labels, metrics, configuration,
                 cluster_stats, run_index, dataset_id, clustering_model):
    '''
    ...
    write_labels_and_assignments(labels_and_assignments_file, clustering_model, data_labels)
...
def save_averaged_results(averaged_metrics, averaged_stats, configuration, args, dataset_id):
    '''
    ...
    '''
    results_file = _get_output_basename(args.output_dir, args.cluster_algorithm, dataset_id)
